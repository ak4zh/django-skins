...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-skins runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-skins.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_skins")
...
def lint(c):
    ...
    """
    c.run("flake8 django-skins tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_skins.__version__))
    c.run("git push --tags")
